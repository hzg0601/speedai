...
### 3.1 deepspeed（分布式，显存优化，量化，MoE）

### 3.2 Transformers Accelerate（分布式训练）

### 3.3 Pytorch-Lightning Fabric(分布式训练)

### 3.4 Apex（混合精度训练，已集成到pytorch）

### 3.5 Pytorch Quantization（pytorch量化包）

### 3.6 PEFT(高效调优，LoRA,Prefix-Tuning等)

https://blog.51cto.com/u_15775105/6221301

https://blog.csdn.net/qq_39388410/article/details/121036309

https://it.sohu.com/a/670981907_121119001

#### 3.6.1 LoRA

LoRA是基于adapter调优启发的低秩自适应调优方法。

adapter方法的原理并不复杂，它是通过在原始的预训练模型中的每个transformer block中加入一些参数可训练的模块实现的。
...
![LoRA（Low-Rank Adaptation of Large Language Models）-- 一种大模型prompt-tuning调优方法_语言模型_02](https://s2.51cto.com/images/blog/202304/22021014_6442d186d8b7871636.png?x-oss-process=image/watermark,size_16,text_QDUxQ1RP5Y2a5a6i,color_FFFFFF,t_30,g_se,x_10,y_10,shadow_20,type_ZmFuZ3poZW5naGVpdGk=/format,webp/resize,m_fixed,w_1184)
...
用户只需要在原来的训练脚本中更改不超过两行的代码，就可以针对不同的下游任务无缝训练新的adapter模块，并且整个adapter模块的参数和原始的预训练模型参数是完全独立存储的。详情可以参阅这个[ 链接](https://github.com/Adapter-Hub/adapter-transformers%20)。
...
* 对于自回归模型，加入前缀后的模型输入表示：**z = [ P R E F I X ; x ; y ] z=[PREFIX;x;y]**z**=**[**PREF**I**X**;**x**;**y**]
* 或者为编码器和编码器都添加prefix：**z = [ P R E F I X ; x ; P R E F I X ′ ; y ] z=[PREFIX;x;PREFIX';y]**z**=**[**PREF**I**X**;**x**;**PREF**I**X**′**;**y**]**。

![在这里插入图片描述](https://img-blog.csdnimg.cn/61d4d84366c142d6919dc385d11acbf5.png#pic_center)
...
https://zhuanlan.zhihu.com/p/475729724
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/9346cbeb713e46c287341e0ddfee0f3a.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBA5LiK5p2J57-U5LqM,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/a69b3d328a5f42679849ff102a5f714a.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBA5LiK5p2J57-U5LqM,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center)
对比起来主要的变动有：
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/9be0683ad4e447ecb00ef9515505e9fc.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBA5LiK5p2J57-U5LqM,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/56075824e10143eb932d4d175489723e.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBA5LiK5p2J57-U5LqM,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/058af10742454f23a3b05e4de5c91590.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBA5LiK5p2J57-U5LqM,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/27cd2ec47bc346a7835e8d0e04b9fc2a.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBA5LiK5p2J57-U5LqM,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center)
**Answer Engineering**
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/1a977853f8974b86b6373af81b9a6932.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBA5LiK5p2J57-U5LqM,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center)
paper：https://arxiv.org/abs/2107.13586

![在这里插入图片描述](https://img-blog.csdnimg.cn/b1126aaf438c42cea3409273cd5c1c33.png?x-oss-process=image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBA5LiK5p2J57-U5LqM,size_20,color_FFFFFF,t_70,g_se,x_16#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/291d5c03b7b84e7a9e9aba70de3d3b9a.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBA5LiK5p2J57-U5LqM,size_15,color_FFFFFF,t_70,g_se,x_16#pic_center)

#### 3.6.5 AdaLoRA

出于rank的秩是固定的，训练完成后不能修改，优化代价大，提出了动态低秩适应（Dy-LoRA）技术。因此为一系列的秩不再是单一秩训练LoRA块，如r在[rmin,rmax]内操作，需要确保增加或减少秩不会明显阻碍模型的性能，通过对适配器模块在训练过程中对不同秩的信息内容进行排序。本质为动态搜索LoRA选择最佳秩，并且推理也是动态的。

AdaLoRA根据重要性评分自适应地分配参数预算。首先以奇异值分解的形式对权重矩阵的增量更新进行参数化，然后根据新的重要性指标，通过操纵奇异值，在增量矩阵之间动态地分配参数预算。这种方法有效地提高了模型性能和参数效率。
